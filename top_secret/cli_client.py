...
class ClientCommandExecutor:
    ...
    def stop(self, process_id):
        ...
        response = requests.post(
            f"{self.service_url}/commands/stop", json={"process_id": process_id}
        )
        return response.json()
    ...
    def status(self, process_id):
        ...
        try:
            ...
            # Debug print to show the status response
        ...
        while True:
            try:
                ...
                if user_input.startswith("start "):
                    ...
                    # Poll for the command's status and output until it's no
                    # longer running
                    while True:
                    ...
                    try:
                        log_response = self.log_client.log(
                            f"Command executed: {command} (process_id: {process_id})"
                        )
                        if (
                            log_response is None
                            or log_response.get("status") != "success"
                        ):
